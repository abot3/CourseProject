...
def IngestPpData(path: str) -> pandas.DataFrame:
    ...
    print("PP Recipes DF name_tokens {}".format(pp_recipes_df['name_tokens'].iloc[:10].values.flatten().tolist()))
    # print("PP Recipes DF name_tokens2 {} {}".format(type(pp_recipes_df['name_tokens'].iloc[0]), pp_recipes_df['name_tokens'].iloc[0]))
    test = [tokenizer.decode(x) for x in pp_recipes_df['name_tokens'].iloc[:20].values]
    print("Tokenizer output{}".format(test))
    # pp_recipes_df['name_tokens', 'ingredient_tokens', 'steps_tokens'].apply(tokenizer.decode(x))
    pp_recipes_df['name_tokens'] = pp_recipes_df['name_tokens'].apply(tokenizer.decode)
    pp_recipes_df.sort_values(['name_tokens'], inplace=True)
    print("PP Recipes DF name_tokens2 {} {}".format(type(pp_recipes_df['name_tokens'].iloc[0]), pp_recipes_df['name_tokens'].iloc[:20]))
...
def IngestRawData(path: str) -> pandas.DataFrame:
    ...
    print("RAW Recipes DF name {} {}".format(type(raw_recipes_df['name'].iloc[0]), raw_recipes_df['name'].iloc[:20]))
    print("RAW Recipes DF description {} {}".format(type(raw_recipes_df['description'].iloc[0]), raw_recipes_df[['name', 'description']].iloc[:5].values.flatten().tolist()))
    print("RAW Recipes DF steps {} {}".format(type(raw_recipes_df['steps'].iloc[0]), raw_recipes_df['steps'].iloc[:5].values.tolist()))
